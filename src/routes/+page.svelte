...
<section class="proyectos">
  <header class="head">
    <h1>Proyectos</h1>
    <button type="button" class="btn-nuevo" onclick={abrir}>+ Proyecto Nuevo</button>
  </header>

  {#if data.proyectos.length === 0}
    <p class="vacio">Aún no tienes proyectos. Crea el primero con “Proyecto Nuevo”.</p>
  {:else}
    <ul class="lista">
      {#each data.proyectos as p (p.id)}
        <li class="item" class:editing={editingId === p.id}>
          ...
          {:else}
            <a class="item-link" href={`/proyectos/${p.id}`}>
...
{#if showModal}
  ...
  <div class="overlay" onclick={cerrar}>
    <div class="modal" role="dialog" tabindex="-1" aria-modal="true" aria-label="Nuevo proyecto" onclick={(e) => e.stopPropagation()}>
