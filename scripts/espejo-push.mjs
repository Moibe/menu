...
// Ruta relativa al home del usuario remoto (root → /root/code/menu). Sirve igual
...
ssh(`export NVM_DIR="$HOME/.nvm" && . "$NVM_DIR/nvm.sh" && nvm use node >/dev/null && pm2 restart menu`);
...
console.log(
	`\nEspejo local → prod completo. Respaldo en el droplet: ${DIR}/backups/prod-${stamp}.db ` +
		`(+ prod-uploads-${stamp}.tgz). Para revertir: copia ese .db de vuelta a ${DIR}/local.db y pm2 restart menu.`
);
